...
Token::Token(const Kind tokenKind, const size_t indexInFile,
             const size_t filePathIndex)
    : m_tokenKind(tokenKind), m_indexInFile(indexInFile),
      m_filePathIndex(filePathIndex), m_contents(nullptr) {
...
Token::Token(Token&& other) noexcept
    : m_tokenKind(other.m_tokenKind), m_indexInFile(other.m_indexInFile),
      m_filePathIndex(other.m_filePathIndex),
...
std::filesystem::path Token::filePath() const {
...
const std::string& Token::contents() const {
  assert(m_tokenContentsMatchKind() &&
         "Attempted to get token contents when there are none.");
  return *m_contents;
}
